...
module.exports = __toCommonJS(test_email_exports);
var import_nodemailer = __toESM(require_nodemailer(), 1);
var test_email_default = async (req) => {
  if (req.method !== "POST") {
  ...
  try {
    ...
    if (!smtpHost || !smtpUser || !smtpPass) {
      return new Response(
        JSON.stringify({
          error: "SMTP environment variables are not configured in Netlify (SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD)."
        }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
